...
console.log(userData.value?.unit)
...
const headers = [
  ...
  { title: 'Nama', key: 'Nama', sortable: false, width: '65%' },
  { title: 'Tipe', key: 'Tipe', sortable: false, width: '15%' },
...
const { data: dataApi, execute: fetchData } = await useApi<any>(
  createUrl('/api/MasterCoa/GetListCoa', { query: queryParams }),
)
...
const openAddDialog = () => {
  currentDetail.value = { Kode: '', Nama: '', Tipe: '' }
...
const handleConfirm = async (isConfirmed: boolean) => {
  ...
  try {
    const res = await $api(`/api/MasterCoa/${selectedData.value.Kode}`, {
      method: 'DELETE',
    })
...
<template>
  ...
  <FormUpload
    ...
    title="Upload Excel COA"
    api-url="/api/MasterCoa/ImportExcel"
    @reload="fetchData"
  />
