...
const jenis = ref([
  { title: 'Aset', value: 'Aset' },
  { title: 'Kewajiban', value: 'Kewajiban' },
  { title: 'Ekuitas', value: 'Ekuitas' },
  { title: 'Pendapatan', value: 'Pendapatan' },
  { title: 'Beban', value: 'Beban' },
])
...
const submitOrderCencon = async () => {
  try {
    ...
    if (cardDetails.value.Tipe === null || cardDetails.value.Tipe === '') {
      snackbarMessage.value = 'Tipe coa harus diisi'
      snackbarColor.value = 'error'
      isSnackbarScaleVisible.value = true
    ...
    const payload = {
      ...
      Tipe: cardDetails.value.Tipe,
    }

    const method = props.isEditMode ? 'PUT' : 'POST'
    const url = props.isEditMode ? `api/MasterCoa/${cardDetails.value.Kode}` : 'api/MasterCoa'
      ...
      <VCardItem class="text-center">
        <VCardTitle>
          <h4 class="text-h4 mb-2">
            {{ props.isEditMode ? `EDIT COA` : 'ADD COA' }}
          </h4>
        </VCardTitle>
      </VCardItem>
